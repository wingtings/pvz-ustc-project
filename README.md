...
| 原创贴图与 reanim 动画 | 已确定局部改图路线，正在整理首批资源 |
| 数值补丁 | 尚未建立可重复构建 |
...
4. 给以后要画的角色建立资源清单，绘画本身继续后置。

文案由[同步工具](tools/README.md)从设计文档生成，测试范围记录在 [v0.2 文案冒烟测试](tests/checklists/v0.2-text-smoke.md)。下一步不是直接做 GPA 钩子，而是先让第一章从选卡到图鉴都能完整游玩。完整安排见[开发路线图](docs/roadmap.md)。
