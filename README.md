...
| 原创贴图与 reanim 动画 | P01 已通过白天实机观察；P02 专注表情与两组蓝白便签已通过原尺寸门禁并进入五替换累计 PAK，实机观察待完成 |
...
4. 首批资源清单已经完成；P01 已进入 v0.5 静态替换验收，实机动画和其余四个槽位仍按独立提交推进。

文案由[同步工具](tools/README.md)从设计文档生成，测试范围记录在 [v0.2 文案冒烟测试](tests/checklists/v0.2-text-smoke.md)。首批美术对象和概念稿见[资源清单](docs/art-resource-inventory.md)。P01 的圆框眼镜与《电磁学千题解》蓝书由差分脚本从本地合法原件生成，不在 Git 中保存带原版像素的合成 PNG；构建细节见[原创素材 PAK 构建](docs/asset-build-pipeline.md)，实机证据见 [P01 白天垂直切片](tests/checklists/v0.5-p01-ingame.md)。v0.3 的[常量补丁框架](docs/v0.3-patching.md)已经能生成和逐字节回滚开发副本，下一步仍要补齐 P04 耐久与跨场景回归，而不是提前进入 GPA 钩子。完整安排见[开发路线图](docs/roadmap.md)。
