...
| 原创贴图与 reanim 动画 | 首批资源、P01 概念稿、P01 头部/前叶及 P04 三档像素契约完成，PAK 增量构建可用，尚未接入游戏 |
...
4. 首批资源清单已经完成；实际贴图替换仍按 v0.5 独立验收。

文案由[同步工具](tools/README.md)从设计文档生成，测试范围记录在 [v0.2 文案冒烟测试](tests/checklists/v0.2-text-smoke.md)。首批美术对象和概念稿见[资源清单](docs/art-resource-inventory.md)。v0.3 的[常量补丁框架](docs/v0.3-patching.md)已经能生成和逐字节回滚开发副本，下一步是补齐实机跨场景回归，而不是提前进入 GPA 钩子。完整安排见[开发路线图](docs/roadmap.md)。
