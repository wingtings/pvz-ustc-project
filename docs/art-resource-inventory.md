...
状态：资源清单和首批五槽位的 16 份像素契约已完成；P01 绿色圆圈通过白天飞行与命中，P02 已补到夜间眨眼，P04 完成白天三档，Z03 通过图鉴与泳池四阶段；Z01 三张袖片通过白天行走和装备受击层级，剩余动作与跨场景还要回归。
...
绿色圆圈使用共享的 28×28 `images/ProjectilePea.png`。差分脚本保留外沿抗锯齿和原本的明暗方向，在中心删除 88 个可见像素，再用深绿内沿、浅绿高光和少量粉笔斑点重画外圈。候选仍有 396 个可见像素，没有向画布新增 Alpha。这个槽位会被普通科豆家族共同使用，寒冰弹丸和火化弹丸则有各自的资源。静态数据见 [`v0.5-p01-green-circle-static.md`](../tests/checklists/v0.5-p01-green-circle-static.md)。白天冒险关已经确认圆圈沿原弹道飞行，透明中心在移动中可辨，命中后也会触发原版受击反馈并消失；火化、普通科豆家族复用和其他背景仍待检查，见 [`v0.5-p01-z01-z03-runtime.md`](../tests/checklists/v0.5-p01-z01-z03-runtime.md)。
