...
状态：资源清单和首批五槽位的 16 份像素契约已完成；P01 绿色圆圈通过飞行、命中、双发与三线复用、暖气片火化，P02 已补到夜间眨眼，P04 完成白天三档，Z03 通过图鉴与泳池四阶段；Z01 三张袖片通过白天行走和装备受击层级，剩余动作与跨场景还要回归。
...
绿色圆圈使用共享的 28×28 `images/ProjectilePea.png`。差分脚本保留外沿抗锯齿和原本的明暗方向，在中心删除 88 个可见像素，再用深绿内沿、浅绿高光和少量粉笔斑点重画外圈。候选仍有 396 个可见像素，没有向画布新增 Alpha。这个槽位会被普通科豆家族共同使用，寒冰弹丸和火化弹丸则有各自的资源。静态数据见 [`v0.5-p01-green-circle-static.md`](../tests/checklists/v0.5-p01-green-circle-static.md)。白天冒险关已经确认圆圈沿原弹道飞行，透明中心在移动中可辨，命中后也会触发原版受击反馈并消失，见 [`v0.5-p01-z01-z03-runtime.md`](../tests/checklists/v0.5-p01-z01-z03-runtime.md)。随后在泳池看到双发同排发出两个圆圈、三线同时覆盖三排，P01 弹丸经过暖气片也会切成未修改的原版火化弹丸；其他普通射手、寒冰弹丸和跨场景仍待检查，见 [`v0.5-p01-family-fire-runtime.md`](../tests/checklists/v0.5-p01-family-fire-runtime.md)。
...
首轮静态候选已经生成。棕色衣料按原明暗映射成米黄色旧纸，纸面加入 `LIM`、极限式、铅笔横线、红色勾号和下划线；白衬衫、红领带、深色轮廓和衣物破口继续使用原像素。候选保持 53×63 画布和 1932 个可见像素，共改动 1134 个像素，旧纸、铅笔灰、批改红分别为 932、157、43 个像素。确定性哈希与 5 项专用测试已经通过，详见 [`v0.5-z01-static.md`](../tests/checklists/v0.5-z01-static.md)。它尚未完成共享骨架实机回归，因此不能把静态结果写成 Z01 或装备型敌人已经实机通过。
